...
from dataset_BPE import BilingualDataset , causal_mask
...
def get_build_tokenizer(config,ds,lang) : 
    tokenizer_path = Path(config['tokenizer_BPE_file'].format(lang)) ## create relative path
...
def greedy_decode(model , source , source_mask , tokenizer_src , tokenizer_tgt , max_len ,device) : 
    sos_idx = tokenizer_src.sos_token_id
    eos_idx = tokenizer_src.eos_token_id
...
def train_model(config):
    ...
    for epoch in range(initial_epoch,config['num_epochs']):
        ...
        for batch in batch_iterator : 
            ...
            loss = loss_fn(proj_output.view(-1,tokenizer_tgt.vocab_size) , target.view(-1))
